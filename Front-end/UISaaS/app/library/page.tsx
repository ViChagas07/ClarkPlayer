...
export default function LibraryPage() {
  ...
  // ── Discover section ────────────────────────────────────
  // Reuses the same useDiscovery() hook as NowPlayingContent (home) —
  // fetches from backend local GET /api/v1/catalog/discovery, cached + fast
  const { data: discoverData, isLoading: discoverLoading } = useDiscovery()
  const discoverTracks: CatalogTrackItem[] = discoverData?.trending_tracks ?? []
  ...
  // Discover data comes from useDiscovery() hook above — backend local,
  // no external API calls, no hardcoded queries, no manual loading state.
